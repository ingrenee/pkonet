 ...
 */
	function init($settings = array()) {
		...
		parent::init(array_merge(
			array(
				'engine' => 'File', 'path' => S2Paths::get('jreviews','S2_CACHE'), 'prefix'=> 's2_', 'lock'=> true,
				'serialize'=> true, 'isWindows' => false
			),
			$settings
		));

		if(!isset($this->__File) && isset($this->settings['path']) && $this->settings['path']) {
			...
			// Create folder and index file if not already there
			$folder = new S2Folder($this->settings['path'], true);

			$file = new S2File($this->settings['path'] . DS . 'index.html', true);

			$this->__File = new S2File($this->settings['path']);
		}
 ...
 */
	function __active() {
		if (!$this->__active && $this->__init && !is_writable($this->settings['path'])) {
			$this->__init = false;
			trigger_error(sprintf(__t('%s is not writable', true), $this->settings['path']), E_USER_WARNING);
		} else {
			$this->__active = true;
		}
		return true;

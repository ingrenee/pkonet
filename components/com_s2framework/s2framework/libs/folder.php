...
/**
 * Folder structure browser, lists folders and files.clas
 *
 ...
 * @subpackage	cake.cake.libs
 */
class S2Folder extends S2Object {
/**
 * Path to Folder.
 *
 * @var string
 ...
 */
	function read($sort = true, $exceptions = false, $fullPath = false) {
		...
		while (false !== ($item = readdir($dir))) {
			...
			$path = S2Folder::addPathElement($this->path, $item);
			if (is_dir($path)) {
	...
	function _findRecursive($pattern, $sort = false) {
		...
		foreach ($files as $file) {
			if (preg_match('/^' . $pattern . '$/i', $file)) {
				$found[] = S2Folder::addPathElement($this->path, $file);
			}
		}
		$start = $this->path;
		foreach ($dirs as $dir) {
			$this->cd(S2Folder::addPathElement($start, $dir));
 ...
 */
	function normalizePath($path) {
		return S2Folder::correctSlashFor($path);
	}
/**
 * Returns a correct set of slashes for given $path. (\\ for Windows paths and / for other paths.)
 *
 ...
 */
	function correctSlashFor($path) {
		if (S2Folder::isWindowsPath($path)) {
			return '\\';
		}
		return '/';
	}
 ...
 */
	function slashTerm($path) {
		if (S2Folder::isSlashTerm($path)) {
			return $path;
		}
		return $path . S2Folder::correctSlashFor($path);
	 }
/**
 ...
 */
	function addPathElement($path, $element) {
		return S2Folder::slashTerm($path) . $element;
	}
/**
 ...
 */
	function inCakePath($path = '') {
		$dir = substr(S2Folder::slashTerm(ROOT), 0, -1);
		$newdir = $dir . $path;
 ...
 */
	function inPath($path = '', $reverse = false) {
		$dir = S2Folder::slashTerm($path);
		$current = S2Folder::slashTerm($this->pwd());
 ...
 */
	function create($pathname, $mode = false) {
		...
		if (is_file($pathname)) {
			$this->__errors[] = sprintf(__t('%s is a file', true), $pathname);
			return false;
		}
		...
		if ($this->create($nextPathname, $mode)) {
			if (!file_exists($pathname)) {
				...
				if (@mkdir($pathname, $mode)) {
					umask($old);
					$this->__messages[] = sprintf(__t('%s created', true), $pathname);
					return true;
				} else {
					umask($old);
					$this->__errors[] = sprintf(__t('%s NOT created', true), $pathname);
					return false;
				}
			}
		}
 ...
 */
	function dirsize() {
		$size = 0;
		$directory = S2Folder::slashTerm($this->path);
 ...
 */
	function delete($path = null) {
		...
		if (is_dir($path) === true) {
			...
			if (is_array($files)) {
				foreach ($files as $file) {
					...
					if (is_file($file) === true) {
						if (@unlink($file)) {
							$this->__messages[] = sprintf(__t('%s removed', true), $file);
						} else {
							$this->__errors[] = sprintf(__t('%s NOT removed', true), $file);
						}
			...
			if (@rmdir($path) === false) {
				$this->__errors[] = sprintf(__t('%s NOT removed', true), $path);
				return false;
			} else {
				$this->__messages[] = sprintf(__t('%s removed', true), $path);
			}
		}
		return true;
	}
 ...
 */
	function copy($options = array()) {
		...
		if (!$this->cd($fromDir)) {
			$this->__errors[] = sprintf(__t('%s not found', true), $fromDir);
			return false;
		}
		...
		if (!is_writable($toDir)) {
			$this->__errors[] = sprintf(__t('%s not writable', true), $toDir);
			return false;
		}
		...
		if ($handle = @opendir($fromDir)) {
			while (false !== ($item = readdir($handle))) {
				if (!in_array($item, $exceptions)) {
					$from = S2Folder::addPathElement($fromDir, $item);
					$to = S2Folder::addPathElement($toDir, $item);
					if (is_file($from)) {
						if (copy($from, $to)) {
							@chmod($to, intval($mode, 8));
							touch($to, filemtime($from));
							$this->__messages[] = sprintf(__t('%s copied to %s', true), $from, $to);
						} else {
							$this->__errors[] = sprintf(__t('%s NOT copied to %s', true), $from, $to);
						}
					}
					...
					if (is_dir($from) && !file_exists($to)) {
						...
						if (@mkdir($to, $mode)) {
							...
							@chmod($to, $mode);
							umask($old);
							$this->__messages[] = sprintf(__t('%s created', true), $to);
							$options = array_merge($options, array('to'=> $to, 'from'=> $from));
							$this->copy($options);
						} else {
							$this->__errors[] = sprintf(__t('%s not created', true), $to);
						}
					}
				}
			}
 ...
 */
	function realpath($path) {
		...
		if (strpos($path, '..') === false) {
			if (!S2Folder::isAbsolute($path)) {
				$path = S2Folder::addPathElement($this->path, $path);
			}
			return $path;
		}
		...
		return S2Folder::slashTerm($newpath);
	}
/**

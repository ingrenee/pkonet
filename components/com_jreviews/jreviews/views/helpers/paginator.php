	...
	function paginate($params)
	{
		...
		if($this->num_pages > $this->num_pages_threshold)
		{
			...
			$this->return = ($this->current_page != 1 && $this->items_total >= 10) ?
				...
				'<span class="jrPageCurrent jrButton jrSmall jrDisabled">'.__t("&laquo;",true).'</span>&nbsp;';
			...
			for($i=1;$i<=$this->num_pages;$i++)
			{
				...
				if($i==1 || $i==$this->num_pages || (in_array($i,$this->range)))
				{
					...
					$this->return .= ($i == $this->current_page) ?
						...
						'<span title="'.sprintf(__t("Go to page %s",true),$i,$i,$this->num_pages).'" class="jrPageCurrent jrButton jrSmall">'.$i.'</span> '
			...
			$this->return .= ($this->current_page != $this->num_pages && $this->items_total >= 10) ?
				...
				'<span class="jrPageCurrent jrButton jrSmall jrDisabled">'.__t("&raquo;",true).'</span>';
		}
		...
		# num_pages_threshold pages or less
			...
			for($i=1;$i<=$this->num_pages;$i++)
			{
				...
				$this->return .= ($i == $this->current_page) ?
					...
					'<span class="jrPageCurrent jrButton jrSmall jrDisabled">'.$i.'</span> '

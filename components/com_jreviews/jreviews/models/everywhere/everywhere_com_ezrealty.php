...
class EverywhereComEzrealtyModel extends MyModel  {

	var $UI_name = 'EZ Realty for Joomla 3.0x';

    var $extension = 'com_ezrealty';

    var $extension_alias = 'com_ezrealty';
    ...
    */
	...
	var $listingUrl = 'index.php?option=com_ezrealty&view=ezrealty&id=%s&Itemid=%s';
	...
	var $fields = array(
		...
		"'com_ezrealty' AS `Listing.extension`",
	...
	var $joins = array(
        'Total'=>"LEFT JOIN #__jreviews_listing_totals AS Totals ON Totals.listing_id = Listing.id AND Totals.extension = 'com_ezrealty'",
		'LEFT JOIN #__ezrealty_catg AS Category ON Listing.cid = Category.id',
		"INNER JOIN #__jreviews_categories AS JreviewsCategory ON JreviewsCategory.id = Listing.cid AND JreviewsCategory.`option` = 'com_ezrealty'",
...
//		$this->fields[] = "'{$this->tag }' AS `Listing.tag`";
	...
	function exists() {
		return (bool) @ file_exists(PATH_ROOT . 'components' . _DS . 'com_ezrealty' . _DS . 'ezrealty.php');
	}
	...
	function afterFind($results) {
		...
		foreach ($results AS $key=>$result) {
			...
			if($images != '' && @file_exists("images/ezrealty/panorama/" . $images)) {
				   $imagePath = "images/ezrealty/panorama/" . $images;
			} else {

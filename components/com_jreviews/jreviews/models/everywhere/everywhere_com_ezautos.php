...
class EverywhereComEzautosModel extends MyModel  {

	var $UI_name = 'EZ Autos for Joomla 3.0x';

    var $extension = 'com_ezautos';

    var $extension_alias = 'com_ezautos';
    ...
    */
	...
	var $useTable = '#__ezautos AS Listing';
    ...
    */
    ...
    var $catTable = 'ezautos_catg';

	/**
	 * Used for listing module - latest listings ordering
	 */

	var $listingUrl = 'index.php?option=com_ezautos&view=ezautos&id=%s&Itemid=%s';

	var $cat_url_param = null;

	var $fields = array(
		'Listing.id AS `Listing.listing_id`',
		'Listing.headline AS `Listing.title`',
		'Listing.cid AS `Listing.cat_id`',
		'Listing.panorama AS `Listing.images`',
        'Listing.cid AS `Category.cat_id`',
		"'com_ezautos' AS `Listing.extension`",
	...
	var $joins = array(
        'Total'=>"LEFT JOIN #__jreviews_listing_totals AS Totals ON Totals.listing_id = Listing.id AND Totals.extension = 'com_ezautos'",
		'LEFT JOIN #__ezautos_catg AS Category ON Listing.cid = Category.id',
		"INNER JOIN #__jreviews_categories AS JreviewsCategory ON JreviewsCategory.id = Listing.cid AND JreviewsCategory.`option` = 'com_ezautos'",
	...
	var $joinsReviews = array(
		'LEFT JOIN #__ezautos AS Listing ON Review.pid = Listing.id',
		'LEFT JOIN #__ezautos_catg AS Category ON Listing.cid = Category.id',
		"INNER JOIN #__jreviews_categories AS JreviewsCategory ON JreviewsCategory.id = Listing.cid AND JreviewsCategory.`option` = 'com_ezautos'",
		'LEFT JOIN #__jreviews_criteria AS Criteria ON JreviewsCategory.criteriaid = Criteria.id'
	);
	...
	function __construct() {
		...
		$this->tag = __t("EZAUTOS_VEHICLE_TAG",true);
...
//		$this->fields[] = "'{$this->tag }' AS `Listing.tag`";
	...
	function exists() {
		return (bool) @ file_exists(PATH_ROOT . 'components' . _DS . 'com_ezautos' . _DS . 'ezautos.php');
	}
	...
	function afterFind($results) {
		...
		foreach ($results AS $key=>$result) {
			...
			if($images != '' && @file_exists("images/ezautos/panorama/" . $images)) {
				   $imagePath = "images/ezautos/panorama/" . $images;
			} else {

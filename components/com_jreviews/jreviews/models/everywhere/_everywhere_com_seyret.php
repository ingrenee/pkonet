...
class EverywhereComSeyretModel extends MyModel  {

    var $UI_name = 'Seyret';

    var $name = 'Listing';

    var $useTable = '#__seyret_items AS Listing';

    var $primaryKey = 'Listing.listing_id';

    var $realKey = 'id';

    var $extension = 'com_seyret';

    var $listingUrl = 'index.php?option=com_seyret&amp;Itemid=%s&amp;task=videodirectlink&amp;id=%s';

    var $cat_url_param = 'catid';

    var $dateKey = 'addeddate';

    var $fields = array(
        'Listing.id AS `Listing.listing_id`',
        'Listing.title AS `Listing.title`',
        'Listing.picturelink AS `Listing.images`',
        "'com_seyret' AS `Listing.extension`",
        'JreviewsCategory.id AS `Listing.cat_id`',
        'Category.categoryname AS `Category.title`',
     ...
     */
    var $joins = array(
        'LEFT JOIN #__seyret_categories AS Category ON Listing.catid = Category.catid',
        "INNER JOIN #__jreviews_categories AS JreviewsCategory ON Category.id = JreviewsCategory.id AND JreviewsCategory.`option` = 'com_seyret'",
    ...
    var $joinsReviews = array(
        "LEFT JOIN #__seyret_items AS Listing ON Review.pid = Listing.id",
        'LEFT JOIN #__seyret_categories AS Category ON Listing.catid = Category.catid',
        "INNER JOIN #__jreviews_categories AS JreviewsCategory ON Category.id = JreviewsCategory.id AND JreviewsCategory.`option` = 'com_seyret'",
        'LEFT JOIN #__jreviews_criteria AS Criteria ON JreviewsCategory.criteriaid = Criteria.id'
    );
    ...
    function __construct() {
        ...
        $this->tag = __t("SEYRET_TAG",true);  // Used in MyReviews page to differentiate from other component reviews

//        $this->fields[] = "'{$this->tag }' AS `Listing.tag`";
    }

    function exists() {
        return (bool) @ file_exists(PATH_ROOT . 'components' . _DS . 'com_seyret' . _DS . 'seyret.php');
    }

    function listingUrl($listing) {

        return sprintf($this->listingUrl,$listing['Listing']['menu_id'],$listing['Listing']['listing_id']);
    ...
    function getNewCategories()
    {
        ...
        $query = "SELECT Component.id AS value,Component.categoryname as text"
        . "\n FROM #__seyret_categories AS Component"
        ...
        . "\n ORDER BY Component.categoryname ASC"
        ;
    ...
    {
        $query = "SELECT Component.id AS `Component.cat_id`,Component.categoryname as `Component.cat_title`, Criteria.title AS `Component.criteria_title`"
        . "\n FROM #__seyret_categories AS Component"

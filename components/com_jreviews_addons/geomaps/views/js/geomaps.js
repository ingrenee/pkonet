...
(function($,undefined) {
	...
	jreviews.geomaps.__api_loading = false;
	...
	jreviews.geomaps.loadAPI = function(callerfn, callback) {
		...
		if(!jreviews.geomaps.__api_loading &&
				(typeof google == 'undefined' || typeof google.maps == 'undefined')) {

			$.getScript(jreviews.geomaps.google_api_url+'&callback=jreviews.geomaps.setAPILoaded');

			jreviews.geomaps.__api_loading = true;
		}
...
(function($) {
	...
	$.widget("jreviews.jrMaps",{
		...
		_init: function() {
			...
			// Support for maps inside widgetkit tabs
			var widgetkittab = self.element.closest('.wk-slideshow');

			if(widgetkittab.length && self.tabs === null)
			{
				self.tabs = true;

				var mapContent = self.element.parents('li'),
					allContent = mapContent.parent('ul').find('li'),
					allTabs = widgetkittab.find('ul.nav'),
		...
		_checkStreetviewStatus: function(latLng) {
			...
			self.sv.getPanoramaByLocation(latLng, 50, function(data, status) {
				...
				if(status == google.maps.StreetViewStatus.OK) {
					...
					self.streetview.setOptions(panoOptions);

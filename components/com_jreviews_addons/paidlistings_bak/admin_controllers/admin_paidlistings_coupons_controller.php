...
class AdminPaidlistingsCouponsController extends MyController
{
    ...
    */
    ...
    /**
    * Returns the section/category json object to build the category tree
    *
    */
    function jsonCategoryTree()
    {
        echo $this->cmsVersion == CMS_JOOMLA15
            ?
            $this->Category->getTree(array(
                'json'=>true,
                'conditions'=>array('Category.id IN (SELECT cat_id FROM #__jreviews_paid_plans_categories)')
            ))
            :
            $this->Category->getCategoryList(array(
                'jstree'=>true,
                'indent'=>false,
...
//                        ,"children"=>$this->getFieldsbyGroupId($group['groupid'])
    ...
    /**
    * Returns the field group ids corresponding to the passed node id which can be a section or catid
    *
    */
    function getGroupIdsByNode()
    {
        ...
        {
            if(strstr($node_id,"s")) {
                $group_ids = explode(",",$this->getGroupIdsBySectionId($node_id));
            }
            else
            {
    ...
    */
    function getGroupIdsByCatId($cat_ids = null)
    {
        ...
        if(strstr($cat_ids,"s")) {
            return $this->getGroupIdsBySectionId($cat_ids);
        }
    ...
    /**
    * Returns the field group ids corresponding to the passed section ids
    *
    * @param array or comma separated string $section ids
    * @return comma separated list of group ids
    */
    function getGroupIdsBySectionId($section_ids = null)
    {
        $group_ids = null;
        $section_ids = str_replace("s","",$section_ids);
        # Get field group ids used for selected categories
        if($section_ids != '')
        {
            $query = "
                ...
                FROM
                    #__jreviews_categories AS JreviewsCat
                LEFT JOIN
                    #__categories AS Cat On Cat.id = JreviewsCat.id AND JreviewsCat.`option` = 'com_content'
                INNER JOIN
                    #__jreviews_criteria AS ListingType ON ListingType.id = JreviewsCat.criteriaid
                WHERE
                    Cat.section IN (".$section_ids.") AND ListingType.groupid > 0
            ";

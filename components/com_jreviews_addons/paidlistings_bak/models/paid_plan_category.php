...
class PaidPlanCategoryModel extends MyModel  {
    ...
    function getPaidSectionIdsArray()
    {
        # Check for cached version
        $cache_file = s2CacheKey('jreviews_paid_sections');

        $paid_cat_ids = $this->getPaidCatIdsArray();

        $paid_section_ids = S2Cache::read($cache_file,'_s2framework_core_');
        ...
        if(!$paid_section_ids) {
            ...
            $query = "
                ...
                WHERE
                    Category.id IN (". implode(',',$paid_cat_ids) .")
            ";

            $paid_section_ids = $this->query($query,'loadColumn');

            S2Cache::write($cache_file, array('section_ids'=>$paid_section_ids),'_s2framework_core_');
        }
        else {

            $paid_section_ids = $paid_section_ids['section_ids'];
        }

        return $paid_section_ids;
    }

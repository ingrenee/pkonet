    ...
    jreviews.paid = {
        ...
        plan: {
            ...
            formSetup: function(page) {
                ...
                if(jreviews.cms == 1) jreviews.paid.plan.submitSection(form);
            ...
            submitCategory: function (form) {
                ...
                form.on('change','.jr-cat-select', function(e) {
                    ...
                    if(jreviews.cms == 1) {
                        ...
                        var selected_section_id = parseInt(form.find('.jr-section-select').val(),10);

                        if(selected_cat_id === 0) {

                            formPlans.fadeOut().delay(100).html();
            ...
            submitSection: function (form) {

                form.find('.jr-cat-select').attr('disabled','disabled');

                form.on('change','.jr-section-select', function(e) {

                    var el = $(this),
                        formChooser = form.find('.jr-paid-categories'),
                        formPlans = form.find('.jr-plans-layout'),
                        catSelect = form.find('.jr-cat-select'),
                        selected_section_id = parseInt(el.val(),10);

                    // Reset category select
                    catSelect.val(0).attr('disabled','disabled');
                    ...
                    var submittingAction = jreviews.dispatch({method:'get',type:'html',controller:'paidlistings_plans',action:'_loadCategories',data:data});

                    submittingAction.done(function(html) {

                        form.find('.jr-form-categories').html(html);
                    });

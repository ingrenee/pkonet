...
class HandlerPaypalComponent extends S2Component
{
    ...
    */
    ...
    function processSubscriptionPlan(&$order,$post_vars)
    {
        ...
        if ($txn_type == 'subscr_payment' && $payment_status == "Completed")
        {
            ...
            if($result) $order = $this->checkSubscriptionRenewal($post_vars, $order);
        }
    ...
    * @param mixed $order
    */
    function checkSubscriptionRenewal($post_vars, $order)
    {
        $txn_id = Sanitize::getString($post_vars,'txn_id');

        if($txn_id == '') return $order;

        $PaidTxnLog = ClassRegistry::getClass('PaidTxnLogModel');

        $txn_duplicate = $PaidTxnLog->findDuplicate($txn_id);

        $plan = & $order['PaidOrder']['plan_info']['plan_array'];

        if(!$txn_duplicate && $order['PaidOrder']['order_status'] == 'Complete') /* The order had already been procesed in the past */
        {
                $PaidTxnLog = ClassRegistry::getClass('PaidTxnLogModel');
